...
document.getElementById("submit")?.addEventListener("click", async () => {
    try {
        // Prepare inputs
        const identity = (document.getElementById("identity") as HTMLInputElement)?.value + "@check_secret";
        ...
        const proofTx = await build_proof_transaction(identity, password, tx_hash, 0, 1);
        const after = Date.now();
        const time = after - before;
        show("logs", `Generated proof transaction in ${(time / 1000).toFixed(2)}sec... ✅`);
