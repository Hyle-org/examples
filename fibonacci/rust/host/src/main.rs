...
fn main() {
    ...
    let receipt = match &cli.command {
        Commands::Next { input, iteration } => prove(cli.reproducible, *input, *iteration),
    ...
    println!("Payload hash {}", u32::from_be_bytes(hyle_output.payload_hash.try_into().unwrap()));
...
fn prove(reproducible: bool, initial_state: u32, suggested_number: u32) -> risc0_zkvm::Receipt {
    ...
    let result = prover.prove(env, &binary).unwrap();
    result
}